...
<%@ page
	import="com.acesoft.aceoffix.*,com.acesoft.aceoffix.wordwriter.*,java.awt.*,javax.servlet.*,javax.servlet.http.*,java.sql.*,java.text.SimpleDateFormat,java.util.Date"%>
...
<%
	...
	String docSubject = "", docName = "", docDept = "", docCause = "", docDays = "", docFile = "", docSubmitTime = "";
	if (rs.next()) {
		...
		docSubmitTime = rs.getString("SubmitTime");
	}
	rs.close();
	stmt.close();
	conn.close();
	...
	doc.openDataRegion("name").setValue(docName);
	doc.openDataRegion("dept").setValue(docDept);
	doc.openDataRegion("cause").setValue(docCause);
	doc.openDataRegion("days").setValue(docDays);
	doc.openDataRegion("date").setValue(docSubmitTime);
	doc.openDataRegion("ToDate").setValue(docSubmitTime);
	//doc.openDataRegion("ToDate").setValue(new SimpleDateFormat("MM/dd/yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(docSubmitTime)));
	doc.openDataRegion("tip").setValue("");
	...
	aceCtrl.addCustomToolButton("Print", "poPrint", 1);
	...
	aceCtrl.openDocument("doc/template.doc", OpenModeType.docReadOnly,
			"Tom");
