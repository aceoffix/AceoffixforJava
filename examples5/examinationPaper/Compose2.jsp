...
<%@ page language="java"
	import="java.util.*,java.sql.*,javax.servlet.*,javax.servlet.http.*,com.acesoft.aceoffix.*,com.acesoft.aceoffix.wordwriter.*"%>
	...
	while (rs.next()) {
		...
		if(chk!=null && chk.equalsIgnoreCase("on")){			
			if(i.equalsIgnoreCase("1")){
				DataRegion dataNum = doc.openDataRegion("begin1");
				dataNum.setValue("1.\t");
				DataRegion dataRegion = doc.createDataRegion("begin"+(i+1),DataRegionInsertType.After,"begin1");
				...
				DataRegion dataRegion = doc.createDataRegion("begin"+(i+1),DataRegionInsertType.After,"ACE_"+num);
				dataRegion.setValue("[word]Openfile.jsp?id=" + i + "[/word]");
			}
			temp = "ACE_begin"+(i+1);
			num ++;
