...
<%@ page import="com.acesoft.aceoffix.*, com.acesoft.aceoffix.wordwriter.*"%>
...
int iRd = rd.nextInt(4);
wd.openDataRegion("StudentName1").setValue(strNames[iRd]);
wd.openDataRegion("StudentName2").setValue(strNames[iRd]);

int iCreditsAttempted = 0;
int iTemp = 0;
for (int i = 0; i < 9; i++){
    int iCredits = rd.nextInt(5) + 1;
    int iScore = rd.nextInt(3) + 2;
    wd.openDataRegion("table4").openTable(1).openCellRC(3 + i, 2).setValue(String.valueOf(iCredits));
    wd.openDataRegion("table4").openTable(1).openCellRC(3 + i, 3).setValue(String.valueOf(iCredits));
    wd.openDataRegion("table4").openTable(1).openCellRC(3 + i, 4).setValue(String.valueOf(strGrades[iScore - 2]));
    iCreditsAttempted = iCreditsAttempted + iCredits;
    iTemp = iTemp + iCredits * iScore;
}
float fGPA = (float)iTemp / iCreditsAttempted;
wd.openDataRegion("table4").openTable(1).openCellRC(12, 1).setValue("Total Credits:  " + String.valueOf(iCreditsAttempted) + "        GPA:       " + String.valueOf(fGPA));

wd.openDataRegion("GPA").setValue(String.valueOf(fGPA));
wd.openDataRegion("CreditsAttempted").setValue(String.valueOf(iCreditsAttempted));
wd.openDataRegion("CreditsEarned").setValue(String.valueOf(iCreditsAttempted));
...
aceCtrl1.setCaption("Generate Word report dynamically.");
aceCtrl1.setFileTitle("Report1");
aceCtrl1.bind(wd);
aceCtrl1.openDocument("../doc/doc-report.doc", OpenModeType.docReadOnly, "John Scott"); // docReadOnly: Disabled Edit, Paste.
...
<li class="page_item menu-item-17"><a href="http://www.aceoffix.com/index.php/about-us/">About Us</a></li>
    ...
    <br/>
    <div class="container left">
        <div class="left"><span>Current Page: <a href="../index.jsp" title="Examples Center">Example Center</a> &gt;</span><span><strong><em>Generate Word report dynamically</em></strong></span></div>
        <div class="right">
